...
import multiprocessing as mp # Import multiprocessing here, at the top
...
def train_episode(agent, env, opponent, self_play=False):
    """Runs a single training episode and returns the experience."""
    state = env.reset()
    done = False
    episode_reward = 0
    experience_buffer = []  # Store experience tuples (state, action, reward, next_state, done)
    ...
    if agent_goes_first:
        while not done:
            ...
            experience_buffer.append((state.copy(), action, reward, next_state.copy(), done)) # Store experience
            ...
            experience_buffer.append((next_state.copy(), opp_action, -opp_reward, next_next_state.copy(), done)) # Store opponent's experience (negative reward)
            episode_reward -= opp_reward # Adjust episode reward for opponent's perspective
            state = next_next_state
    else:
        # Opponent goes first (similar structure, adapt experience storage)
        while not done:
            # Opponent's turn
            if self_play:
            ...
            experience_buffer.append((state.copy(), opp_action, -opp_reward, next_state.copy(), done)) # Opponent's experience (negative reward)
            ...
            experience_buffer.append((next_state.copy(), action, reward, next_next_state.copy(), done)) # Agent's experience
            episode_reward += reward
            state = next_next_state

    return experience_buffer, episode_reward


def train_agent(env, agent, opponent, num_episodes=10000, num_processes=4, decay_exploration=True, self_play=False):
    """Parallelized training function."""
    pool = mp.Pool(processes=num_processes)
    metrics = {'win_rates': [], 'rewards': [], 'episode_lengths': []} # Initialize metrics

    for episode_batch_start in range(0, num_episodes, num_processes):
        episode_batch_end = min(episode_batch_start + num_processes, num_episodes)
        episodes_in_batch = episode_batch_end - episode_batch_start

        # Prepare arguments for parallel episode runs
        episode_args = [(agent, env, opponent, self_play) for _ in range(episodes_in_batch)]

        # Run episodes in parallel
        results = pool.starmap(train_episode, episode_args)  # Use starmap for multiple arguments

        # Aggregate experience and rewards from all processes
        batch_experience = []
        batch_rewards = []
        for experience, reward in results:
            batch_experience.extend(experience)
            batch_rewards.append(reward)

        # Update agent with aggregated experience (in the main process)
        for state, action, reward, next_state, done in batch_experience:
            agent.learn(state, action, reward, next_state, done)

        # Calculate and update metrics (in the main process)
        avg_reward = np.mean(batch_rewards)
        metrics['rewards'].extend(batch_rewards) # Append individual rewards, or consider averaging over batch

        # Decay exploration and update target network (less frequently now, per batch)
        if (episode_batch_start // num_processes + 1) % 1 == 0: # Decay every batch (adjust frequency)
            agent.decrease_exploration()
            if isinstance(agent, DeepQAgent):
                agent.update_target_model()

        # Periodically update win rate and save (less frequently)
        if (episode_batch_start // num_processes + 1) % 10 == 0:  # Check win rate less frequently
            metrics['win_rates'].append(agent.get_win_rate())
            agent.save()
            print(f"Episodes {episode_batch_start + 1}-{episode_batch_end}/{num_episodes} - Avg Reward (Batch): {avg_reward:.2f}, Win rate: {agent.get_win_rate():.2f}, Exploration: {agent.exploration_rate:.4f}")
