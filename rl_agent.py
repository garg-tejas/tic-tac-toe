...
def train_agent(env, agent, opponent, num_episodes=10000, decay_exploration=True):
    """Train the RL agent against an opponent"""
    for episode in range(num_episodes):
        state = env.reset()
        done = False
        episode_reward = 0
        
        # Determine who goes first - increased probability for agent to go first during training
        # This helps the agent learn more winning strategies from the first position
        if random.random() < 0.7:  # 70% chance agent goes first instead of 50%
            # Agent goes first
            while not done:
                # Agent's turn
                ...
                # Agent learns from opponent's action
                # Penalize losses more harshly to encourage defensive play
